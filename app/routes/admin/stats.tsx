...
export default createRoute(async (c) => {
  ...
  const protocolCounts = {
    routerpay: hasOrderData ? orderRows.filter((order) => order.inboundProtocol === "routerpay").length : statsData.breakdown.protocol.routerpay,
    easypay: hasOrderData ? orderRows.filter((order) => order.inboundProtocol === "easypay").length : statsData.breakdown.protocol.easypay
  };
  const statusCounts = {
    created: hasOrderData ? orderRows.filter((order) => order.status === "created").length : statsData.breakdown.status.created,
    pending: hasOrderData ? orderRows.filter((order) => order.status === "pending").length : statsData.breakdown.status.pending,
    paid: hasOrderData ? paidOrders.length : statsData.breakdown.status.paid,
    failed: hasOrderData ? orderRows.filter((order) => order.status === "failed").length : statsData.breakdown.status.failed,
    refunded: hasOrderData ? orderRows.filter((order) => order.status === "refunded").length : statsData.breakdown.status.refunded
  };
  const paidOrderCount = hasOrderData ? paidOrders.length : statsData.summary.paidCount;
  const exportHref = createStatsExportHref(statsData, filters);
    ...
    <AdminShell title="统计面板">
      <div class="space-y-6">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric label="成功率" value={`${statsData.summary.successRate}%`} detail={`${paidOrderCount} 笔已支付`} icon="chart" tone="warning" />
        ...
        <div class="grid gap-6 xl:grid-cols-[1.4fr_0.6fr]">
          ...
          <ProtocolDistribution routerpay={protocolCounts.routerpay} easypay={protocolCounts.easypay} />
        </div>
          ...
          <Section title="订单状态">
            <div class="p-5">
              <MiniBarChart
                ...
                values={[statusCounts.created, statusCounts.pending, statusCounts.paid, statusCounts.failed, statusCounts.refunded]}
              />
            </div>
          </Section>
          <Section title="渠道分布">
...
function StatsFilters(props: { filters: StatsFiltersValue; providers: string[]; exportHref: string; error?: string }) {
      ...
      <form class="stats-filters" method="get" action="/admin/stats">
        ...
        <div class="stats-filter-row stats-filter-row-dimensions">
          ...
          <div class="stats-actions">
            ...
            <a class="toolbar-link" href={props.exportHref} download="routerpay-stats.csv">导出</a>
          </div>
        </div>
      </form>
...
function createStatsExportHref(statsData: ReturnType<typeof createMockStats>, filters: StatsFiltersValue) {
  const rows = [
    ["维度", "值"],
    ["时间范围", formatRangeLabel(statsData.from, statsData.to)],
    ["指标", metricTitle(filters.metric)],
    ["金额", String(statsData.summary.amountMinor / 100)],
    ["订单数", String(statsData.summary.orderCount)],
    ["成功率", `${statsData.summary.successRate}%`],
    ["回调成功率", `${statsData.summary.callbackRate}%`],
    [],
    ["时间", "数值"],
    ...statsData.points.map((point) => [point.label, String(point.value)]),
    [],
    ["失败原因", "次数", "占比"],
    ...statsData.failureReasons.map((reason) => [reason.label, String(reason.count), `${reason.rate}%`])
  ];
